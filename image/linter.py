...
class ManifestLayersSupportRequiredMediaTypes(
        LintRule[ContainerImageManifest]
    ):
    """
    A lint rule ensuring a manifest's layers support the required media types
    """
    def lint(
            self,
        ...
        for layer in artifact.get_layer_descriptors():
            ...
            if layer_media_type not in expected_media_types:
                return LintResult(
        ...
        return LintResult(
            message="All layers support expected mediaTypes: " + \
                str(expected_media_types)
        )
...
class ManifestListSupportsRequiredPlatforms(
    ...
    def lint(
        ...
        missing = [
            set(required).difference(platforms)
        ]
        if len(missing) > 0:
            return LintResult(
                status=LintStatus.ERROR,
                message=f"({self.name()}) " + \
                    "manifest list does not support required platforms: " + \
...
class ManifestListSupportsRequiredMediaTypes(
    ...
    def lint(
        ...
        return LintResult(
            message="Manifest list and manifests support expected maediaTypes"
        )
