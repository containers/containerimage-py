...
3. Locate the ``.whl`` (wheel) file in the ``dist`` folder. It should be named something like so: ``containerimage_py-1.0.0-py3-none-any.whl``
4. Run the following command from the root of the repository, replacing the name of the ``.whl`` file if necessary

.. code-block:: shell

   pip install dist/containerimage_py-1.0.0-py3-none-any.whl
